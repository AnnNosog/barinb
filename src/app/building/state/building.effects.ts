...
import { createBuildingFromNewBuilding } from '../common/building.util'
import { BuildingStorage } from '../../building/storage/building.storage'
import { EnvironmentService } from '../../core/environments/environment.service';
//import { fetch } from '@app/core/store/utils';
import { fetch } from '../../core/store/utils/data-persistence.util';
  ...
  removeBuildingRoom$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        ...
        run: (action, buildingsEntities) => {
          const building = buildingsEntities ? buildingsEntities[action.payload.id] : null;
          const rooms = building?.rooms.filter((room: number) => room !== action.payload.room) ?? [];
