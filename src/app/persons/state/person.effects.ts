...
import { createPersonFromNewPerson } from '../common/person.util';
import { PersonStorage } from '../storage/person.storage';
  ...
  removePersonBuilding$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        ...
        run: (action, personsEntities) => {
          const person = personsEntities ? personsEntities[action.payload.id] : null;
          const buildings = person?.buildings.filter((building: number) => building !== action.payload.building) ?? [];
